...
1. **`PingProcessor::loadIRFromFile`:** if the stem ends in one of the aux suffixes:
   ...
   - **Base sibling missing** (orphan) → load only that file into its matching `MicPath` slot via `loadIRFromBuffer(..., MicPath::{Direct,Outrig,Ambient})`. MAIN and the other aux slots are left untouched, so the user keeps whatever set they already had and can layer the orphan aux on top.
2. **`PingEditor::importIR`:** if the picked file's stem ends in an aux suffix:
   ...
   - **No base sibling in source** → copy the orphan file into the user IR folder but **rename it by dropping the suffix** so the import lands as a coherent MAIN file. The next time it's loaded it behaves as a normal MAIN IR; no dangling aux files with no MAIN remain on disk.
