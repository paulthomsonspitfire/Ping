...
- **Preset combo + Save button** — right-aligned at `w - 12` margin; preset combo fixed at 200 px wide, label reads **"PRESET"** at 14 pt. See Preset section below.
...
- **Header panel spans the full window width** — `headerPanelRect = Rectangle(0, 0, w, topRow.getBottom())` is set in `resized()` and painted first in `paint()` before logos or waveform. The steel gradient and reflection sweep are applied to this rect. Spitfire logo is fixed at x=14 (not aligned to knob columns). P!NG logo uses `ping_logo_blue_png`, sized by its 578:182 aspect ratio to fill the header height, drawn at 1× directly into `pingBounds`. The old `spitfireBounds.setX(irInputGainSlider.getX())` re-alignment has been removed — do not add it back.
