...
**Current version:** 2.4.5 (see `CMakeLists.txt`)
...
- **Dirty detection uses `AudioProcessorValueTreeState::Listener`, not `ValueTree::Listener`** — the processor registers as an APVTS listener for every parameter (iterating `getParameters()` + `dynamic_cast<RangedAudioParameter*>`). The original `ValueTree::Listener` on `apvts.state` failed after `replaceState()` because the listener was disconnected from the new tree's parameter children. APVTS parameter listeners survive `replaceState` because they are registered by parameter ID, not by tree node.
