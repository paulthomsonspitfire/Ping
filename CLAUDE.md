...
Tools/
  ...
  generate_factory_irs.cpp      — Batch factory IR + preset generator from hardcoded VENUES[]
                                  (developer only; refuses to overwrite existing .ping sidecars
                                   unless --overwrite-sidecars is explicitly passed, v2.9.1+)
  rebake_factory_irs.cpp        — Sidecar-driven re-bake of existing factory .wav IRs from their
                                  .ping files (v2.9.1+). Never touches .ping. Preferred whenever
                                  the goal is "apply engine-code changes to the current factory set."
                                  Supports --dry-run, --only <substring>, -q/--quiet.
  fix_factory_preset_paths.py   — Rewrites `irFilePath` in factory preset XMLs to point at the
                                  system install location `/Library/Application Support/Ping/
                                  Factory IRs/...` (v2.9.2+). See "Factory preset `irFilePath`
                                  convention" below.
...
## Floor plan — Option-mirror puck drag (v2.6.1, axis selector v2.7.7)

### What it does

Holding **Option** (Alt) while dragging any puck on `FloorPlanComponent` snaps the paired puck to a mirror position around either the room's **vertical** centre line (x = 0.5, the historical default — useful for L/R pairs) or the **horizontal** centre line (y = 0.5, useful for front/back pairs). Makes it trivial to place mic and speaker pairs symmetrically without nudging each one individually. Works for all four pairs — speakers (0/1), MAIN mics (2/3), OUTRIG mics (4/5), AMBIENT mics (6/7) — and applies to both position drags (puck body) and rotation drags (ring handle).

The axis is chosen from a pair of small icon buttons in the IR Synth Room Geometry panel (see "Axis selector UI" below) and persists in the `.ping` sidecar.

### Behaviour

- **Partner index is always `dragIndex ^ 1`** — XOR with 1 flips the LSB, mapping 0↔1, 2↔3, 4↔5, 6↔7. Cheap, no lookup table.
- **Position mirror — vertical axis (default):** `partnerCx = 1.0 − draggedCx`, `partnerCy = draggedCy` (same y).
- **Position mirror — horizontal axis:** `partnerCx = draggedCx` (same x), `partnerCy = 1.0 − draggedCy`.
- **Angle mirror — vertical axis:** `partnerAngle = π − draggedAngle`. Verified against the default Mic L/R pair: L = −2.356 rad (up-left), R = −0.785 rad (up-right); `π − (−2.356) = −0.785` ✓. Also verified for speakers at π/2 (down) → π − π/2 = π/2 (also down) ✓.
- **Angle mirror — horizontal axis:** `partnerAngle = −draggedAngle` (reflection across the x axis). For an L mic at −2.356 rad (up-left), the partner snaps to +2.356 rad (down-left), preserving the side of the room and flipping the front/back orientation.
- **Angle mirroring is rotation-drag only** — same as the position rule: when the user drags the *body* of a puck the partner mirrors position only; when the user drags the *ring handle* the partner mirrors angle only. The user has to actively rotate the dragged puck for the partner's angle to snap, so passively translating an L mic around the room never overwrites the R mic's angle.
...
- **Axis is also latched at `mouseDown`** — `mirrorDragAxis` captures the active `mirrorAxis` preference at the moment Option-down fires, so the cursor glyph, dashed guide, and position/angle math all stay consistent for the rest of the drag even if the user clicks the other axis button mid-drag.
- **L-shaped room guard** — if the mirrored point fails `isInsideRoom (mx, my)`, the partner holds its last valid position while the dragged puck keeps moving freely. When the dragged puck returns to a symmetric spot the partner resumes updating. The dragged puck itself is still constrained to the room via the existing `isInsideRoom` check. Applies to both axes.
...
Two small (18×18 px) custom-painted toggle buttons sit under the Width/Depth/Height sliders in the IR Synth Room Geometry section, with an "Option-mirror" label on the left. The buttons are radio-grouped (`setRadioGroupId(8211)`) so exactly one is always active.

- **Vertical button** — rounded rectangle with a vertical dashed line (`{2.5, 2.0}` dash). When active, fill is `accentIce` at 16% alpha, outline at 95%, line in full accent.
- **Horizontal button** — same rectangle with a horizontal dashed line.

Hovering shows a tooltip explaining the axis ("Option-drag mirrors mics across the vertical centre line (left / right)" / "...horizontal centre line (front / back)"). Both buttons live in `IRSynthComponent` and write directly to `floorPlanComponent.mirrorAxis`; clicking either also fires `onParamModifiedFn` so the IR-synth-dirty flag flips and the change round-trips through `getParams`/`setParams`.

The implementation is a private nested class `IRSynthComponent::MirrorAxisButton` defined in `IRSynthComponent.cpp` (file scope, fully-qualified). Because the class is forward-declared in the header but defined in the .cpp, `~IRSynthComponent` must be defined out-of-line in the .cpp (`IRSynthComponent::~IRSynthComponent() = default;` immediately above the ctor) so the `unique_ptr<MirrorAxisButton>` destructor can see the complete type.
...
1. **Dashed centre guide** — a 1 px dashed line is drawn at the active mirror axis (vertical at x = 0.5 or horizontal at y = 0.5) in the plugin's icy-blue accent (`0xff8cd6ef`, 45% alpha, dash pattern `{ 6, 4 }`). Drawn inside the grid's clip region so it sits inside the room walls, not outside. Only visible during the drag (`mirrorDrag == true`); cleared on `mouseUp`. The orientation is driven by `mirrorDragAxis` (the latched-at-mousedown axis), so it stays consistent for the duration of the drag.
2. **Custom cursor** — a 32×32 ARGB image built procedurally by `FloorPlanComponent::makeMirrorCursor (bool horizontal)`: either a vertical axis line with two triangular arrows pointing outward (`← | →`) or a horizontal axis with arrows pointing up and down (`↑ — ↓`), filled in accent ice-blue with a soft black outline so it reads on any background. Hotspot at (16, 16) so the glyph sits over the puck being dragged. Both glyphs are built once in the constructor and cached as `juce::MouseCursor mirrorCursorVertical` / `mirrorCursorHorizontal` members — not re-rasterised per drag. `mouseDown` selects the appropriate cursor based on `mirrorDragAxis`.
...
The active axis is stored in `IRSynthParams::mirror_axis` (int, 0 = vertical, 1 = horizontal) and round-trips through `irSynthParamsToXml` / `irSynthParamsFromXml` in `PluginProcessor.cpp` as the `mirrorAxis` XML attribute. Defaults to 0 for older sidecars that lack the attribute. The parameter is **UI-only** — the synthesis engine never reads it; it only controls future Option-drag behaviour.

`IRSynthComponent::getParams` reads `floorPlanComponent.mirrorAxis` into `p.mirror_axis`; `setParams` writes it back to both the floor plan (so the next drag uses the correct axis) and the two toggle buttons (so the UI reflects the stored value). All standard `setParams` notification suppression applies (`suppressingParamNotifications` flag + button `dontSendNotification`).

### DSP/engine impact

**None.** The mirror feature only writes to `transducers.cx[]`, `transducers.cy[]`, and `transducers.angle[]` — the same fields a normal drag writes. `IRSynthEngine` reads `IRSynthParams::source_* / receiver_* / micl_angle / micr_angle` which are sourced from `TransducerState` by `IRSynthComponent::buildParamsFromState()`, so the engine is completely unaware of how the values got there. No test impact (no IR_* or DSP_* golden values shift). `mirror_axis` is also strictly UI-only — even though it is persisted in `IRSynthParams`, no engine code path reads it.

### Key design decisions — floor plan mirror

- **`dragIndex ^ 1` works because pairs are always adjacent even/odd** — the `TransducerState` layout (`0/1 = speakers, 2/3 = MAIN, 4/5 = OUTRIG, 6/7 = AMBIENT`) is fixed by convention. Do not reorder these indices or the XOR trick breaks and every mirror pairing would need an explicit lookup. The same partner-index rule applies to both axes — only the position/angle formulas differ.
- **Option AND axis are both latched at mousedown** — `mirrorDrag = e.mods.isAltDown() && ...` and `mirrorDragAxis = mirrorAxis` are stored once in `mouseDown` and never re-read for the rest of the drag. This protects against two independent kinds of glitch: (a) accidentally toggling symmetry by glancing off the Option key, and (b) clicking the opposite axis button mid-drag (which would otherwise instantly relocate the partner across a different axis). Both glitches were rejected as visually jarring; if the user wants to switch axis they release the mouse and start a new drag.
- **Angle mirroring is rotation-drag only, by design** — when the user drags the puck *body* the `for (int s = 0; s < 2; ++s)` rotation-handle path doesn't fire, so the partner's angle is left untouched. This means dragging an L mic around the room never overwrites the R mic's facing — the user has to actively grab the rotation ring to mirror angles. The same rule applies to both axes.
- **Horizontal-axis angle is `−angle`, not `−angle + π`** — reflecting an angle across the x axis (y = 0.5 mirror line) is `−θ` in the standard JUCE convention where 0 = +x and π/2 = +y. For an L mic facing −2.356 rad (up-left, into the back of the room), the partner snaps to +2.356 rad (down-left, into the front of the room), preserving the *side* of the room and flipping the front/back orientation — which is exactly what a horizontal-axis mirror should do. Do not introduce a `+π` term: that would also flip the side, collapsing horizontal mirror into vertical mirror plus rotation.
- **L-shaped fallback = skip partner, not clamp both** — when the mirrored point is outside the room, the dragged puck keeps moving freely and the partner just holds its last valid position. Alternatives rejected: (a) constraining the dragged puck so the partner stays inside prevents the user from placing a mic anywhere in the L's asymmetric arm, breaking the whole point of mirroring; (b) letting the partner sit outside the room corrupts `IRSynthParams` and would be rejected by the engine's reflection geometry anyway. Same fallback applies to both axes.
- **Mirror cursors are built once at construction** — both `mirrorCursorVertical` and `mirrorCursorHorizontal` are constructed in the `FloorPlanComponent` ctor via `makeMirrorCursor (false/true)`. Earlier revisions tried lazy-init on first use (`if (! mirrorCursor.getHandle())`), but `juce::MouseCursor` has no stable public API for "is this an image cursor?" and rebuilding on every drag is wasteful. The two 32×32 ARGB images total < 10 KB — trivial to keep resident.
...
- **`MirrorAxisButton` forces `IRSynthComponent` destructor out-of-line** — `MirrorAxisButton` is forward-declared in `IRSynthComponent.h` and only fully defined in `IRSynthComponent.cpp` so it stays a private implementation detail. `unique_ptr<MirrorAxisButton>` requires the complete type at its destructor call site, so `IRSynthComponent::~IRSynthComponent() = default;` must be defined in the `.cpp` (above the ctor) where `MirrorAxisButton` is fully visible. If the destructor is declared but not defined, you'll get `error: invalid application of 'sizeof' to an incomplete type 'IRSynthComponent::MirrorAxisButton'` from `__memory/unique_ptr.h`. Do not move the button class into the header just to avoid the out-of-line destructor — the header pollution isn't worth it.
- **Mirror axis is persisted in `IRSynthParams` despite being UI-only** — there is no separate UI-state file in this plugin; the `.ping` sidecar is the only persistent per-IR state. Folding `mirror_axis` into `IRSynthParams` (rather than inventing a parallel persistence channel) means it round-trips for free through every existing save/load path, including embedded base64 sidecars in DAW session state. The engine simply ignores fields it doesn't read, so this has zero engine impact and zero test impact.
...
- **`setStateInformation` / `loadIRFromFile` pre-clear unused mic paths via `clearMicPath` — do not remove this** — Without clearing, loading a preset that doesn't ship a particular path (e.g. a MAIN-only preset after a full multi-mic preset) leaves the previous preset's `rawSynth*Buffer`, `*IRLoaded` flag, convolver IR, and `pathDisplayName` slot intact. The strip's power switch on `MicMixerComponent` is gated on `isPathIRLoaded(path)`, so it stays *enabled* with the stale flag — the user can flip on e.g. the OUTRIG strip after loading a MAIN-only preset and audibly hear the prior preset's outrig IR. Fix: `clearMicPath(MicPath)` (private helper on `PingProcessor`) wipes the per-path raw buffer + atomic `*IRLoaded` flag + display name (`<empty>`); for `MicPath::Main` it additionally clears `currentIRBuffer` / `selectedIRFile` / `lastLoadedIRFile` / `irFromSynth`. Three call sites: (a) `setStateInformation` clears all four paths up-front (BEFORE restoring `selectedIRFile` from XML, since clearing MAIN wipes it) and then loads only what the preset actually contains; missing `<synthIR*>` children stay no-ops because `loadAuxSynthChild` early-returns and the slot remains cleared. (b) `loadMicPathFromFile` calls `clearMicPath(path)` when the suffix-derived sibling WAV is missing, so picking a base IR with no `_outrig.wav` sibling clears any prior outrig content instead of silently leaving it. (c) `loadIRFromFile`'s orphan-aux branch (user picks `Foo_outrig.wav` directly with no base sibling) clears MAIN + the other two aux paths before loading the orphan into its slot, giving clean "you picked one file, you get one path" semantics. Critically `clearMicPath` does NOT touch the `juce::dsp::Convolution` objects: `processBlock` already gates each path on `*IRLoaded` (`if (mainOnRaw && mainIRLoaded.load() && mainReady)`), so once the flag is `false` the stale convolver contents are unreachable, and no convolver `reset()` from the message thread means no risk of racing the audio thread or the NUPC background load. The same masking applied to `applyMixerGates` (sidecar gate flags ANDed with `isPathIRLoaded(path)` so a sidecar requesting `outrigOn=true` doesn't enable a parameter the front-panel control would have to grey out anyway).
...
- **`LOADED:` filename row sits between the Mic Paths strip and the bottom bar (v2.7.9)** — A 16 px row carved off the *top* of `barArea` (with a 4 px gap below it before the bar contents) shows which file currently backs each `MicPath` slot. Layout: a `LOADED:` caption (dim grey, 11 pt, `loadedRowLabel`) anchored to `stripX` on the left, then four `pathNameLabels[4]` centred under the four columns of `layoutMicPathsStrip` using identical column geometry (4 equal columns, 10 px gap, 6 px inset — re-derived from `micStripArea` so the labels track any future strip-column change without exposing the column rectangles as members). Each label is tinted with its mixer accent — MAIN `0xff8cd6ef` (icy blue), DIRECT `0xffe87a2d` (warm orange), OUTRIG `0xffc987e8` (soft violet), AMBIENT `0xff7bd67b` (fresh green) — kept in sync with `MicMixerComponent` `kAccentMain/Direct/Outrig/Ambient`. Carving from `barArea` (rather than `contentArea`) was deliberate: the bar's button row uses only `barY+6 → barY+30` of its 52 px allocation and has ~22 px of bottom slack, so reclaiming 20 px (16 row + 4 gap) from the top shifts every bar control down without overflowing past `getLocalBounds()` and without touching the floor plan, left controls, or strip above. Do not move the row into `contentArea` — it would shrink the floor plan, and the user requested the floor plan stay put.
- **`PingProcessor::getPathIRDisplayName(MicPath)` is the single source of truth for the LOADED row text** — Returns `"<empty>"`, `"<unsaved>"`, or the loaded file's stem (aux suffix preserved for aux paths so the user can verify which file actually backs each slot, e.g. `Venue_outrig`). The processor caches four `juce::String pathDisplayName[4]` members (read/written only on the message thread, so plain `juce::String` is safe — no atomic needed). Setters fire from every load site that touches a `MicPath` slot: `loadIRFromFile` MAIN branch (sets MAIN to the file stem), `loadIRFromFile` orphan-aux branch (sets the matched aux slot to the orphan's stem with suffix), `loadMicPathFromFile` (sets the aux slot to the sibling's stem when a sibling exists), `loadIRFromBuffer` `fromSynth` path (sets the synthesised slot to `"<unsaved>"` until `finishSaveSynthIR` writes the WAV and the resulting load updates it to the saved stem), and `prepareToPlay` (resets all four to `"<empty>"`). The editor binds `IRSynthComponent::setPathDisplayNameSupplier` to a lambda calling this getter, polled by the IR Synth panel's existing GUI timer (cheap compare-and-set, only repaints when text changes). Do not move the supplier wiring out of the editor constructor or duplicate the cache in `IRSynthComponent` — the processor is the only correct owner because load events originate there.
...
- **IR Synth dirty indicator (" *" asterisk) on IR synth combo** — `IRSynthComponent` has a local `dirty` flag and a `cleanIRName` member. `PingProcessor` stores `irSynthDirty` (atomic bool) for cross-editor-lifecycle persistence. All IR synth parameter controls (sliders, combos, erOnly toggle, FloorPlanComponent placement drag) wire change notifications through `onParamModifiedFn`, which sets both `irSynthComponent.dirty` and `processor.irSynthDirty`. Dirty is cleared on IR load from combo, IR save (`finishSaveSynthIR`). After Calculate IR, dirty is set to true (the IR is fresh but unsaved). The timer (4 Hz) appends/strips " *" on `irCombo` using the same popup-only guard as the main combo (see "Dirty indicator display guard" key design decision). When Save is clicked, an `AlertWindow` always prompts for the save name (pre-filled with the current combo text with any " *" stripped, or "My IR" if empty) — matching the `ensurePresetNamed` behaviour for preset save. Do not revert to the old dirty-only prompt: without the unconditional prompt, saving a synth IR while the combo text happened to show a factory-IR name (e.g. immediately after loading a factory preset and then hitting Calculate IR without interacting with the IR combo) would silently try to write over that factory stem. `FloorPlanComponent` fires `onPlacementChanged` in `mouseUp` when a transducer was being dragged. The synth-parameter combos (`shapeCombo`, `floorCombo`, `ceilingCombo`, `wallCombo`, `vaultCombo`, `micPatternCombo`, plus `outrigPatternCombo`, `ambientPatternCombo`, `directMaxOrderCombo`) are registered as `ComboBox::Listener` on `IRSynthComponent`; `comboBoxChanged` fires `onParamModifiedFn` for any combo that is not `irCombo`.
- **IR synth Sample Rate picker removed in v2.8.x** — the Options strip previously held a `sampleRateCombo` offering 44100 / 48000 as a render rate for the synthesised IR. This was a vestige of the pre-plugin web-app era where the user downloaded a WAV at the chosen rate. Inside the plugin it had no effect on playback: `juce::dsp::Convolution::loadImpulseResponse` always resamples the IR to the rate `prepareToPlay` received from the host, so the picker only changed the native rate of *exported* WAVs and the band-split internals inside `IRSynthEngine`. Removed to eliminate user confusion ("can I run Ping at 44.1?" — yes, always). `IRSynthParams::sample_rate` is retained in the struct and round-tripped via the `sr` XML attribute so old presets load bit-identically; `IRSynthComponent::paramsFromUI` now pins `p.sample_rate = 48000` unconditionally for fresh renders. Do not reinstate the combo — if a user needs to export at 44.1 k, add it as an option on the IR *export* dialog instead of mixing it into the synth-params UI.
- **Late Calculate-IR results are discarded if a new IR loaded in the meantime (v2.9.3)** — `synthIR()` runs on a `juce::ThreadPool` job and can take tens of seconds. If the user clicks **Calculate IR**, then switches presets (or loads an IR file, or opens a session) before the job completes, the async completion handler used to silently stomp the newly loaded IR with the stale result. Fix: `PingProcessor` exposes `setIRReplacedFromFileCallback(std::function<void()>)`; `loadIRFromFile` and `setStateInformation` both fire it at entry. `PingEditor` registers a lambda that calls `IRSynthComponent::invalidatePendingSynth()` which sets an atomic `pendingSynthInvalidated = true`. The `MessageManager::callAsync` completion inside `IRSynthComponent::startSynthesis` checks that flag before touching the processor: if set, it updates the progress label to "Cancelled (preset changed)." and bails out. `startSynthesis` clears the flag at the start of each new job. Do not move the callback firing out of `loadIRFromFile`/`setStateInformation` (those are the only two paths that replace the convolver's IR without going through the IR Synth panel), do not replace the atomic with a plain bool (the synth job runs on a background thread), and do not remove the re-clear at the start of `startSynthesis` (fresh jobs must not inherit the previous cancellation).
- **IR Synth UI is fully interaction-locked while a Calculate IR job is running (v2.9.4)** — `IRSynthComponent::setInteractionLocked(bool)` toggles `setEnabled` on every child control: all sliders, combos, buttons including **Save IR** and **Main Menu**, and the FloorPlanComponent. Called with `true` at the top of `startSynthesis`, and with `false` from the `callAsync` completion handler (regardless of whether the result was accepted or discarded due to the race guard above). This prevents three distinct failure modes: (a) the user clicks "Main Menu" mid-job and returns to the main editor while the late synth result still arrives and clobbers whatever IR is now live; (b) the user tweaks a parameter mid-job and expects the next Calculate to honour the new value (it wouldn't — the running job has already captured params); (c) the user clicks Save IR before the buffer is ready and writes a silent file. The lock deliberately covers the Main Menu button — the earlier approach of "only disable params but let navigation through" still hit failure mode (a) because `setStateInformation` / preset loads could fire from the main editor while the IR Synth panel was unmounted. Do not add exceptions for specific controls without re-auditing all three failure modes.
...
### Factory preset `irFilePath` convention (v2.9.2+)

Factory preset XMLs embed an `irFilePath` attribute pointing at the `.wav` they were saved with. **For shipped factory presets this MUST be the system install location** (`/Library/Application Support/Ping/Factory IRs/<subfolder>/<name>.wav`), never a developer-machine path like `/Users/<me>/.../Installer/factory_irs/...`.

Why this matters: the installed plugin tries `irFilePath` first when loading a preset. If the path resolves on disk (e.g. an old stem still exists under the developer's local working copy on the dev machine), the plugin will load **that** WAV instead of the system-wide copy. In v2.9.0 this masked the polygon sign-bug fix locally — presets opened to stale pre-fix WAVs because the dev-machine path still existed, while "Calculate IR" re-ran the current engine and produced the corrected output. The symptom was "Load and Calculate sound different on the same sidecar."

`Tools/fix_factory_preset_paths.py` is the surgical fix. It decodes each preset's `VC2!`-wrapped XML, rewrites `irFilePath` to the system path, re-encodes with the correct size field, and writes back. Use it any time factory IR locations change, any time a preset is authored on a developer machine and needs to be shipped, or as a belt-and-braces step before every installer build. The installer pre-flight (`Installer/build_installer.sh`, v2.9.1+) prints a summary of any `irFilePath` values that are not under the system install root so this never silently regresses again.

### Factory content workflow — which tool to use

- **User edits an existing factory sidecar and wants new WAVs** → `build/rebake_factory_irs` (safe: only touches `.wav`s).
- **User wants to add a brand new venue preset** → hand-author a `.ping` sidecar under `Installer/factory_irs/<subfolder>/`, then `build/rebake_factory_irs --only "<name>"`.
- **Engine code changed and all factory WAVs need regenerating** → `build/rebake_factory_irs` (NOT `generate_factory_irs` — that would overwrite hand-edited sidecars).
- **Preset XML `irFilePath` values drift** → `python3 Tools/fix_factory_preset_paths.py`.
- **Starting from scratch, bootstrapping a new VENUES[] entry into the codebase** → `generate_factory_irs` is still authoritative for seeding a .ping + .wav pair from the hardcoded parameters, but must be invoked deliberately (see "Agent guardrails" at the top of this document).

Building the installer alone (`cmake --build build --target installer` or `Installer/build_installer.sh`) never regenerates content — it only reads the committed `Installer/factory_irs/` + `Installer/factory_presets/` trees into a staging payload and trims the staging copy in place.
...
When a user selects `Venue.wav` from either combo, `PingProcessor::loadIRFromFile` auto-loads the three sibling files via `loadMicPathFromFile(file, MicPath::{Direct,Outrig,Ambient})` — siblings are derived by appending `_direct` / `_outrig` / `_ambient` to the MAIN file's stem within the same directory. Siblings that don't exist on disk are skipped silently (fully backward-compatible with pre-multi-mic IRs). The sidecar beside the MAIN file is the sole source of truth for which strips are active: `setParams` applies the enabled synthesis flags to the UI, and the `<mixerGates>` child element (see below) applies the four mixer strip On booleans so the mixer reproduces the user's intended bus configuration.

### Mixer gate state in the `.ping` sidecar (v2.7.6+)

The MAIN sidecar now carries an optional `<mixerGates mainOn="…" directOn="…" outrigOn="…" ambientOn="…"/>` child element next to `<synthParams …/>`. This is **independent of** the `*_enabled` flags inside `<synthParams>`: `*_enabled` controls whether the engine *synthesises* that path; the mixer gates control whether the per-path convolver *contributes to the wet bus* at playback time.

- **Writer:** `PingProcessor::writeSynthIRSetToDirectory` calls `getCurrentMixerGates()` and passes it into `writeIRSynthParamsSidecar(..., &gates)` so every saved IR remembers how the mixer was configured when the IR was authored.
- **Reader:** `PingProcessor::tryLoadMixerGatesFromSidecar(const juce::File& irFile, MixerGateState& out)` returns `true` if the element exists. `PingEditor` consumes it in two places: (a) the IR Synth panel's IR-load callback, and (b) `importIR` after the copy succeeds. Both branches guard with `! pingProcessor.getIsRestoringState()` so a preset restore's APVTS state always wins — mixer gates saved in a preset are not overridden by the IR's sidecar copy.
- **Apply:** `applyMixerGates(const MixerGateState&)` writes to the four APVTS parameters via `setValueNotifyingHost`, so the host, undo manager, and audio thread all see the change atomically.
- **Backwards compat:** older sidecars that lack `<mixerGates>` return `false` from `tryLoadMixerGatesFromSidecar` and the mixer is simply left in its current state. Struct defaults (`mainOn=true`, others=false) only apply to brand-new `MixerGateState` instances.

### Aux-suffix routing in `loadIRFromFile` / `importIR` (v2.7.6+)

Previously, picking an aux file (e.g. `Venue_outrig.wav`) directly — via the main combo (only possible when aux is orphaned), via `setStateInformation`'s stem fallback, or via Import — would silently load it as a MAIN IR. That was intentional fallback behaviour but caused two footguns: (a) a set partially on disk (e.g. only the outrig + ambient WAVs survived) would come back into MAIN instead of their right slots; (b) an orphan aux imported from AirDrop would permanently land as a phantom MAIN without the user realising.

The routing is now:

1. **`PingProcessor::loadIRFromFile`:** if the stem ends in one of the aux suffixes:
   - **Base sibling present** in the same directory → recurse into `loadIRFromFile(baseFile)` so MAIN + `_direct` + `_outrig` + `_ambient` auto-load together, identical to the user having clicked the base directly.
   - **Base sibling missing** (orphan) → load only that file into its matching `MicPath` slot via `loadIRFromBuffer(..., MicPath::{Direct,Outrig,Ambient})`, **and auto-enable the corresponding mixer-strip `*On` parameter** via `setValueNotifyingHost` so the load is immediately audible. MAIN and the other aux slots are left untouched, so the user keeps whatever set they already had and can layer the orphan aux on top. (Skipped during preset restore — the preset's APVTS state wins.)
2. **`PingEditor::importIR`:** if the picked file's stem ends in an aux suffix:
   - **Base sibling present in the source directory** → re-anchor `src` on the base file, so `copyIRSetWithSiblings` imports the whole coherent set.
   - **No base sibling in source** (orphan) → **preserve the aux suffix on copy** so the file lands as `<destStem><suffix>.wav` in the user folder, and additionally copy the source dir's base sidecar (`<srcBaseStem>.ping`, if present) to `<destStem>.ping`. The orphan stays an aux file on disk, so subsequent loads route via case 1 above into the correct `MicPath` slot.
3. **Sidecar lookup transparently strips the aux suffix.** `PingProcessor::getSidecarFor(file)` returns the path to `<baseStem>.ping`, so `loadIRSynthParamsFromSidecar` and `tryLoadMixerGatesFromSidecar` both pick up the base sidecar even when called with an aux-suffixed file. This is why the IR Synth panel can show pucks / room geometry for an orphan aux load when the base sidecar is present.
4. **Mixer-gate sidecar application is skipped for aux-suffixed loads.** The editor calls `tryLoadMixerGatesFromSidecar` only when `! PingProcessor::hasAuxSuffix(file)`. Reason: a sidecar's gate state was captured for the *full set* (e.g. `mainOn=true, outrigOn=true, ambientOn=true`); applying it for an orphan-aux load would also try to enable strips whose IR was never loaded, leading to dimmed-but-on strips and misleading visual state. The orphan path uses `loadIRFromFile`'s auto-enable (case 1) instead, which only flips the one strip whose IR actually loaded.

This is the sole place the aux-routing logic lives, so both the combo-click path and the import path produce identical results.
...
**Orphaned aux files (base MAIN `.wav` missing):** If the user manually deletes `Venue.wav` (or receives only the aux files via AirDrop) so that `Venue_outrig.wav` and `Venue_ambient.wav` exist without `Venue.wav`, the `scanFolder` aux-sibling filter does not hide them and both remain visible in the combos. Since v2.7.6 the load path detects the aux suffix and routes the content to its matching `MicPath` slot rather than loading it into MAIN — so clicking `Venue_outrig.wav` lands in the OUTRIG strip, not MAIN. See "Aux-suffix routing in `loadIRFromFile` / `importIR`" above.
...
- **Multi-mic aux siblings (`_direct` / `_outrig` / `_ambient`) are hidden from both combos when the MAIN base exists** — filtered in `IRManager::scanFolder` Pass 3, so both `getEntries()` and `getEntries4Channel()` see the same filtered list. `loadIRFromFile` already auto-loads the siblings when the MAIN file is selected, and the MAIN `.ping` sidecar's `<mixerGates>` element drives the mixer strip gates (since v2.7.6). See "Multi-mic IR save/load naming convention" in the IRManager section. Orphaned aux files (no base sibling) are left visible and, since v2.7.6, route to their matching `MicPath` slot on selection rather than loading into MAIN. Do not move this filter into `refreshIRList` alone — it needs to apply equally to the IR Synth combo.
- **Mixer gate state (`<mixerGates>`) is independent of `IRSynthParams.*_enabled`** — `*_enabled` controls *synthesis* (whether the engine renders that path in `synthIR`); the mixer gate booleans control *playback* (whether the per-path convolver contributes to the wet bus). Both are persisted in the `.ping` sidecar but in separate XML children so a user who synthesised with OUTRIG+AMBIENT enabled but muted the mixer strips gets exactly that state reproduced on reload. During a preset restore the `.ping` gates are ignored (guarded by `! isRestoringState()`) so the preset's APVTS state wins — the sidecar's copy is only used when the user explicitly selects an IR or imports one outside of preset-load context.
- **Aux-suffix routing lives in `loadIRFromFile` and `importIR`, not deep in `setStateInformation`** — `setStateInformation` restores `selectedIRFile` by path or stem lookup, then calls `loadIRFromFile`. Putting the aux-suffix redirect logic inside `loadIRFromFile` means preset restores, direct combo clicks, imports, and the stale-path stem fallback all get the same aux routing for free. The routing is: base sibling present → recurse on the base; base sibling missing → load orphan into its matching `MicPath` slot **and auto-enable that strip's `*On` APVTS parameter** so the load is audible. On import, orphans **preserve their aux suffix on disk** (do not rename to MAIN) so they continue to load via the aux-routing branch on every subsequent click — this is what lets the IR Synth panel display the right pucks / room geometry from the base sidecar.
- **Sidecar-lookup helper `PingProcessor::getSidecarFor(file)` is the single entry point for `.ping` resolution** — it transparently strips any aux suffix on `file`'s stem so a click on an orphan `Venue_outrig.wav` correctly resolves to `Venue.ping` (the only place a sidecar is ever written). Both `loadIRSynthParamsFromSidecar` and `tryLoadMixerGatesFromSidecar` use it, as do the editor's three sidecar-`existsAsFile` checks (combo selection, IR Synth panel selection, import). Don't re-introduce the old `file.getSiblingFile(file.getFileNameWithoutExtension() + ".ping")` pattern at any new site — it silently misses the base sidecar for aux-suffixed files.
- **Mixer-gate application is gated on `! PingProcessor::hasAuxSuffix(file)`** — the editor's three sidecar-load sites call `tryLoadMixerGatesFromSidecar` only when the loaded file is itself a MAIN (full-set load). Applying full-set gates for an orphan-aux load would re-enable strips that have no IR loaded, leaving the mixer in a misleading "dimmed but enabled" state. The orphan path's auto-enable inside `loadIRFromFile` is the correct, scoped alternative — it flips on only the strip whose IR was actually loaded.
