...
- **Frequency-dependent mic polar patterns** — `micG()` takes a `band` parameter (0–7, corresponding to octave bands 125 Hz–16 kHz). The `s_mic` map stores `std::array<std::pair<double,double>, 8>` per pattern (one `{omni, directional}` pair per band). The constraint `o + d = 1.0` is maintained at every band so on-axis gain is frequency-flat; only off-axis rejection varies with frequency. `micG` is called **once per reflection per band** from inside the `for (int b = 0; b < N_BANDS; ++b)` loop in `calcRefs`. The `"cardioid"` key is kept as a backward-compatible alias for the same data as `"cardioid (LDC)"` so older saved presets load correctly; `IRSynthComponent::setParams` additionally migrates the legacy display string to the new `"cardioid (LDC)"` label. The five patterns are: omni (flat 1.00/0.00), subcardioid (0.85/0.15 at 125 Hz → 0.50/0.50 at 16 kHz), cardioid LDC (0.78/0.22 at 125 Hz → 0.06/0.94 at 16 kHz — significant narrowing above 1 kHz), cardioid SDC (0.65/0.35 at 125 Hz → 0.18/0.82 at 16 kHz — more consistent directivity across frequency), figure-8 (0.12/0.88 at 125 Hz → 0.00/1.00 from 1 kHz up). `micFaceAngle` (from `p.micl_angle` / `p.micr_angle`) is unchanged — mic direction from the FloorPlan UI continues to rotate the pattern correctly. `FloorPlanComponent` treats any non-omni/non-subcardioid/non-figure8 pattern as the default cardioid arc, so both `"cardioid (LDC)"` and `"cardioid (SDC)"` draw the correct visual beam. IR_11's onset index is unchanged (still 482) because speaker/mic geometry is identical; only per-band sample values shifted — see the IR_11 golden values comment.
